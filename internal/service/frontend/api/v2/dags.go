...
func (a *API) ListDAGs(ctx context.Context, request api.ListDAGsRequestObject) (api.ListDAGsResponseObject, error) {
	...
	var result execution.PaginatedResult[*core.DAG]
	var errList []string
	var err error

	if statusFilter != "" {
		// Get all DAGs without pagination for status filtering
		allPaginator := execution.NewPaginator(1, math.MaxInt)
		allDagsResult, allErrList, allErr := a.dagStore.List(ctx, execution.ListDAGsOptions{
		...
		result = execution.NewPaginatedResult(paginatedDags, totalCount, pg)
	} else {
