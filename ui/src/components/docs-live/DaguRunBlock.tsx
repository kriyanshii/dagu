...
export function DaguRunBlock({ source }: Props) {
  ...
  // Outside the docs page (no provider) the block is an inert summary.
  if (!live) {
    return (
  ...
  const execute = async () => {
    ...
    showToast(parsed.mode === 'enqueue' ? 'DAG run enqueued' : 'DAG run started');
  };
      ...
      <div className="flex items-center gap-2">
        ...
        {runEnabled && (
          ...
          >
            ...
            {posting ? 'Starting…' : parsed.mode === 'enqueue' ? 'Enqueue' : 'Run'}
          </button>
        )}
      </div>
      <ParamsTable params={parsed.params} />
